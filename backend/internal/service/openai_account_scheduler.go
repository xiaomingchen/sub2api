...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		account := accounts[i].CloneWithEffectivePriority(req.GroupID)
		if req.ExcludedIDs != nil {
	...
	for _, account := range filtered {
		fresh := s.service.resolveFreshSchedulableOpenAIAccount(ctx, account, req.RequestedModel, false)
		...
		if req.ExcludedIDs != nil {
			if _, excluded := req.ExcludedIDs[fresh.ID]; excluded {
				continue
			}
		}

		loadInfo := loadMap[fresh.ID]
		if loadInfo == nil {
			loadInfo = &AccountLoadInfo{AccountID: fresh.ID}
		}
